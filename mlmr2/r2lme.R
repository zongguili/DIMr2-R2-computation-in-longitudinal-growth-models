...
m1<-lme(reading~time+WM+sex+time*sex,random=~1+time|CHILDID,data=dat_wmnona)

## Example 2 VC
psych::describe(dat$WM)
dat$gmcWM<-dat$WM-480.05
m2 =lme(reading ~ time+gmcWM+sex, data = dat,
        random = ~ 1+time| CHILDID,
        weights = varIdent(form = ~ 1 |time),
        control=lmeControl(maxIter = 1e8, msMaxIter = 1e8)
)
summary(fm)
resfm<-(c(1.0000000, coef(fm$modelStruct$varStruct, unconstrained=F))*fm$sigma)^2
...
m2<-lme(reading~time+WM,random=~1|CHILDID,data=df)

## example 2-1
effectf<-c("time")
effectr<-NULL

r2lme(m2,effectf,effectr)
...
m3<-lme(reading~1,random=~1|CHILDID,data=df)

effectf<-NULL
effectr<-NULL

r2lme(m3,effectf,effectr)