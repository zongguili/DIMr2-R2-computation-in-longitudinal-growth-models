...
m1<-lme(reading~time+WM,random=~1+time|CHILDID,data=df)

effectf<-c("WM")
effectr<-NULL

r2lmeRmat(m1,effectf,effectr)


## Example 2

m2<-lme(reading~time+WM,random=~1+time|CHILDID,data=df,
...
m3<-lme(reading~time+WM,random=~1+time|CHILDID,data=df,
...
r2lmeRmat(m3,effectf,effectr)


## Example 4

m4<-lme(reading~time+WM,random=~1+time|CHILDID,data=df,
        correlation=corCompSymm(form=~time|CHILDID),
        weights=varIdent(form=~1|time),
        control=lmeControl(maxIter=1e8,msMaxIter=1e8))
...
m5<-lme(reading~time+sex+time*sex,random=~1+time|CHILDID,data=df,
...
m6<-lme(reading~time+WM+sex+time*sex,random=~1|CHILDID,data=df,
        correlation=corAR1(form=~time|CHILDID),
        weights=varIdent(form=~1|time),
        control=lmeControl(maxIter=1e8,msMaxIter=1e8))
...
## Example 7 (random intercept model)

m7<-lme(reading~1,random=~1|CHILDID,data=dat)

effectf<-NULL
effectr<-c("(Intercept)")

r2lmeRmat(m7,effectf,effectr)
